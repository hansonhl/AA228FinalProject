...
function discrete_pomdp(config)
...
function value_iteration(config)
    m_disc, _ = discrete_pomdp(config)
    println(m_disc)
    ...
    save_path = "vi_mdp_discrete_$(config).jld"
    save(save_path, "policy", policy)
end


function get_mcts_policy(config)
    _, m = discrete_pomdp(config)
    ...
    solver = POMCPSolver(tree_queries=5000, c=10, estimate_value=estimate_value_mdp(p_mdp))
    println(solver)
